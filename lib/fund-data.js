...
function getComparisonMetricWindow(series, rangeKey = null, endDate = null) {
  ...
  if (!rangeKey || rangeKey === "MAX") {
    ...
    const periodSeries = series.filter((point) => (
      getTimestamp(point.date) >= getTimestamp(startPoint.date)
      && getTimestamp(point.date) <= getTimestamp(endPoint.date)
    ));
  ...
  const startPoint = resolveBoundaryPointWithPreference(series, startDate, { preference: "nearest" });
...
export function buildComparisonMetrics(funds, rangeKey = "1Y") {
  ...
  const profitabilityRows = [
    ...
    buildMetricRow(`Rentabilidad anualizada · ${periodLabel}`, comparableFunds, (fund) => {
      const periodWindow = getPeriodWindowForFund(fund);
      return createPercentCell(
        periodWindow
          ? computeAnnualizedReturn(periodWindow.startPoint, periodWindow.endPoint)
          : null
      );
    }),
  ];
  ...
  return {
    note:
      `Las metricas del periodo ${periodLabel} se anclan a la ultima fecha comun disponible entre los fondos comparados y, si no existe dato exacto en un borde, usan la observacion mas cercana dentro de una tolerancia de 7 dias.`,
    sections: [
      {
        title: "Rentabilidad",
      ...
      },
      {
        ...
        rows: [
          ...
          buildMetricRow(`Ratio retorno/volatilidad · ${periodLabel}`, comparableFunds, (fund) => {
            const periodWindow = getPeriodWindowForFund(fund);
            const periodSeries = getPeriodSeries(fund);
            const annualizedReturn = periodWindow
              ? computeAnnualizedReturn(periodWindow.startPoint, periodWindow.endPoint)
              : null;
