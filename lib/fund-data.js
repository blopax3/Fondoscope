...
const MORNINGSTAR_ID_PATTERN = /(0P[A-Z0-9]+|F0[A-Z0-9]+)/i;
const ISIN_PATTERN = /^[A-Z]{2}[A-Z0-9]{9}[0-9]$/;

function normalizeFundIdentifierToken(token) {
  const rawToken = String(token || "").trim();
  if (!rawToken) {
    return "";
  }

  const morningstarMatch = rawToken.match(MORNINGSTAR_ID_PATTERN);
  if (morningstarMatch) {
    return morningstarMatch[1].toUpperCase();
  }

  const sanitized = rawToken.replace(/[^A-Za-z0-9]/g, "").toUpperCase();
  return ISIN_PATTERN.test(sanitized) ? sanitized : "";
}
