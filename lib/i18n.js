const LANGUAGE_CONFIG = {
  es: {
    htmlLang: "es",
    locale: "es-ES",
    metadataDescription: "Histórico de fondos por ISIN en Morningstar con Yahoo Finance como alternativa",
    dashboard: {
      subtitle: "Consulta y compara fondos de inversión por ISIN",
      loadSectionTitle: "Carga de fondos",
      loadSectionDescription: "Consulta primero en Morningstar. Si no hay histórico, se usará el símbolo de Yahoo Finance que indiques para ese fondo.",
      isinLabel: "ISIN de los fondos (Morningstar)",
      currencyLabel: "Divisa",
      yahooSymbolLabel: "Símbolo de Yahoo Finance (opcional)",
      yahooSymbolHint: "Mismo fondo y clase. Solo se usa si falta el histórico de Morningstar, en la divisa publicada por Yahoo.",
      sourceLabel: "Fuente",
      invalidIsins: "ISIN no válidos (no se admiten URLs ni IDs de Morningstar)",
      invalidYahooSymbol: "Símbolo de Yahoo no válido. Introduce el símbolo, no una URL.",
      placeholder: "Ej.: IE00B4L5Y983 · Un ISIN por línea, o separados por comas",
      removeTitle: "Eliminar",
      maxFundsHint: (maxFunds, overflowCount) =>
        `Máximo ${maxFunds} fondos · Elimina ${overflowCount} entrad${overflowCount > 1 ? "as" : "a"} para continuar`,
      selectedFundsHint: (count) =>
        `${count} fondo${count > 1 ? "s" : ""} · Selecciona la divisa de cada fondo`,
      emptyHint: "Separadores: línea, coma, espacio o punto y coma. Solo ISIN de 12 caracteres con dígito de control válido.",
  ...
  en: {
    htmlLang: "en",
    locale: "en-US",
    metadataDescription: "Fund history by ISIN from Morningstar with Yahoo Finance as a fallback",
    dashboard: {
      subtitle: "Look up and compare investment funds by ISIN",
      loadSectionTitle: "Fund input",
      loadSectionDescription: "Look up Morningstar first. If history is unavailable, use the Yahoo Finance symbol you specify for that fund.",
      isinLabel: "Fund ISINs (Morningstar)",
      currencyLabel: "Currency",
      yahooSymbolLabel: "Yahoo Finance symbol (optional)",
      yahooSymbolHint: "Same fund and share class. Used only when Morningstar history is unavailable, in Yahoo’s quoted currency.",
      sourceLabel: "Source",
      invalidIsins: "Invalid ISINs (URLs and Morningstar IDs are not accepted)",
      invalidYahooSymbol: "Invalid Yahoo symbol. Enter a symbol, not a URL.",
      placeholder: "E.g. IE00B4L5Y983 · One ISIN per line, or separated by commas",
      removeTitle: "Remove",
      maxFundsHint: (maxFunds, overflowCount) =>
        `Maximum ${maxFunds} funds · Remove ${overflowCount} entr${overflowCount > 1 ? "ies" : "y"} to continue`,
      selectedFundsHint: (count) =>
        `${count} fund${count > 1 ? "s" : ""} · Select a currency for each fund`,
      emptyHint: "Separators: newline, comma, space, semicolon. Only 12-character ISINs with a valid check digit.",
